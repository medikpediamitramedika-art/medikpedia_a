            ...
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <select name="sediaan" class="form-input {{ $errors->has('sediaan') ? 'is-invalid' : '' }}">
                        ...
                        <option value="box"          {{ old('sediaan') == 'box'          ? 'selected' : '' }}>Box — Kotak kardus luar pembungkus obat</option>
                        <option value="fls"          {{ old('sediaan') == 'fls'          ? 'selected' : '' }}>Fls / Flacon — Botol kaca/plastik untuk cairan/sirup</option>
                        <option value="pot"          {{ old('sediaan') == 'pot'          ? 'selected' : '' }}>Pot — Wadah tabung pendek bermulut lebar untuk salep/krim</option>
                        <option value="tube"         {{ old('sediaan') == 'tube'         ? 'selected' : '' }}>Tube — Tabung fleksibel untuk salep/krim</option>
                        <option value="blister"      {{ old('sediaan') == 'blister'      ? 'selected' : '' }}>Blister / Strip — Lembaran plastik/aluminium untuk tablet/kapsul</option>
                        <option value="sachet"       {{ old('sediaan') == 'sachet'       ? 'selected' : '' }}>Sachet — Kantong kecil sekali pakai untuk bubuk/cair</option>
                        <option value="ampul"        {{ old('sediaan') == 'ampul'        ? 'selected' : '' }}>Ampul — Wadah kaca segel runcing sekali pakai untuk injeksi</option>
                        <option value="vial"         {{ old('sediaan') == 'vial'         ? 'selected' : '' }}>Vial — Botol kaca kecil tutup karet untuk injeksi dosis ganda</option>
                        <option value="catch_cover"  {{ old('sediaan') == 'catch_cover'  ? 'selected' : '' }}>Catch Cover — Kertas lipat pembungkus strip obat</option>
                        <option value="suppositoria" {{ old('sediaan') == 'suppositoria' ? 'selected' : '' }}>Suppositoria — Obat padat berbentuk peluru untuk anus</option>
                        <option value="ovula"        {{ old('sediaan') == 'ovula'        ? 'selected' : '' }}>Ovula — Obat padat berbentuk telur untuk vagina</option>
                        <option value="enema"        {{ old('sediaan') == 'enema'        ? 'selected' : '' }}>Enema — Botol pipa panjang untuk cairan pencahar anus</option>
                        <option value="inhaler"      {{ old('sediaan') == 'inhaler'      ? 'selected' : '' }}>Inhaler / Nebules — Tabung semprot atau cairan untuk obat hirup</option>
                        <option value="pen"          {{ old('sediaan') == 'pen'          ? 'selected' : '' }}>Pen — Alat suntik berbentuk pena (contoh: insulin)</option>
                        <option value="pcs"          {{ old('sediaan') == 'pcs'          ? 'selected' : '' }}>Pcs — Satuan per buah/unit</option>
                    </select>
                    @error('sediaan')
                        <div class="form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                    @enderror
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="number" name="terjual" class="form-input {{ $errors->has('terjual') ? 'is-invalid' : '' }}"
                               placeholder="0" min="0" value="{{ old('terjual', 0) }}">
                        @error('terjual')
                            <div class="form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" name="grade" class="form-input {{ $errors->has('grade') ? 'is-invalid' : '' }}"
                               placeholder="Contoh: A / B / C" value="{{ old('grade') }}">
                        @error('grade')
                            <div class="form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                        @enderror
                    </div>
